...
`
const MessageWrap: React.FC<ToastInfo> = ({ message, title, explorer }) => {
  return (
...
const CloseButton = ({ closeToast }: { closeToast: MouseEventHandler<HTMLDivElement> }) => (
  <CloseWrap className="material-icons" onClick={closeToast}>
    <SvgClose />
  </CloseWrap>
)
const SuccessToast: React.FC<ToastInfo> = ({ message, title = 'Success', explorer }) => {
  return (
    <SuccessWrap>
      <SvgSuccess />
...
const ErrorToast: React.FC<ToastInfo> = ({ message, title = 'Failed', explorer }) => {
  return (
    <ErrorWrap>
      <SvgError />
      <MessageWrap message={message} title={title} explorer={explorer} />
...
const WarningToast: React.FC<ToastInfo> = ({ message, title = 'warning', explorer }) => {
  return (
    <WarningWrap>
      <SvgWarning />
      <MessageWrap message={message} title={title} explorer={explorer} />
...
const InfoToast: React.FC<ToastInfo> = ({ message, title = 'Info', explorer }) => {
  return (
    <InfoWrap>
      <SvgInfo />
      <MessageWrap message={message} title={title} explorer={explorer} />
...
const toastify = {
  success(toastInfo: ToastInfo | string, options?: ToastOptions<{}> | undefined) {
    ...
    toast.success(<SuccessToast message={message} title={title} explorer={explorer} />, {
      bodyClassName: '',
  ...
  error(toastInfo: ToastInfo | string, options?: ToastOptions<{}> | undefined) {
    const { message, title, explorer } = handleToastParams(toastInfo)
    toast.error(<ErrorToast message={message} title={title} explorer={explorer} />, {
      bodyClassName: '',
      position: 'bottom-center',
      autoClose: 2000,
  ...
  warn(toastInfo: ToastInfo | string, options?: ToastOptions<{}> | undefined) {
    const { message, title, explorer } = handleToastParams(toastInfo)
    toast.warning(<WarningToast message={message} title={title} explorer={explorer} />, {
      bodyClassName: '',
      position: 'bottom-center',
      autoClose: 2000,
  ...
  info(toastInfo: ToastInfo | string, options?: ToastOptions<{}> | undefined) {
    const { message, title, explorer } = handleToastParams(toastInfo)
    toast.info(<InfoToast message={message} title={title} explorer={explorer} />, {
      bodyClassName: '',
      position: 'bottom-center',
      autoClose: 2000,
